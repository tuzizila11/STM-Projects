...
static boolean_T OverrunFlags[2] = { false, };
...
int main (void)
{
  ...
  while (1) {
    ...
    if (remainAutoReloadTimerLoopVal_S == 0) {
      ...
      if (OverrunFlags[0]) {
        rtmSetErrorStatus(BalancingRobot_M, "Overrun");
      }
       ...
       */
      BalancingRobot_SetEventsForThisBaseStep(eventFlags);

      /* Step the model for base rate */
      BalancingRobot_step(0);
